...
/*************************************
Description: This constructor populates the 8x8 array as NULL. Also wraps walls around the sides, and creates 2 doors and 1 item.
*************************************/
Stairway::Stairway(player *player, string inputNameOfRoom) : Spaces(player, inputNameOfRoom)
{
    rows = 8;
    columns = 8;
...
/*************************************
Description: Deconstructor will delete the array,  Walls.
*************************************/
Stairway::~Stairway()
{
...
/*************************************
Description: Will pull the inputted name of room Stairway.
*************************************/
void Stairway::getNameOfRoom()
{
cout << nameOfRoom << endl;
}

/*************************************
Description: When the player gets to array 7, 1 it will move the player to the space below Library
*************************************/
int Stairway::moveDownRoom()
{
    int number =  0;
...
/*************************************
Description: When the player gets to the door at 0, 1 it will move the player to the space above Dungeon Level 1.
*************************************/
int Stairway::moveUpRoom()
{
    int number =  0;
...
/*************************************
Description: When the player comes down from the Dungeon Level one, sets the player at array 1, 1.
*************************************/
void Stairway::setPlayerDown()
{
    rooms[1][1] = player1;
}

/*************************************
Description: When the player comes up from the Library, sets them at array 6, 1.
*************************************/
void Stairway::setPlayerUp()
{
    rooms[6][1] = player1;
}
...
/*************************************
Description: Prints the particular room.
*************************************/
void Stairway::printRoom()
{
...
*************************************/
void Stairway::movePlayer(player player)
{
    cout << "1. Move up." << endl;
    ...
    if (choice == 1)
    {
		// verifies if the player goes north and theres a wall above.
        if (rooms[currentRowOfPlayer-1][currentColumnOfPlayer] != NULL)
        {
			...
			//the door is locked, the player can't move past
			...
			//player is allowed to pass through since the door is unlocked now
            if (rooms[currentRowOfPlayer-1][currentColumnOfPlayer]->getTileType() == 'D')
            {
                cout << "";
            }
        }
        findPlayerLocation(rooms, rows, columns); //checks where the player is
        rooms[currentRowOfPlayer-1][currentColumnOfPlayer] = player1; //new player location
        rooms[currentRowOfPlayer][currentColumnOfPlayer] = NULL; //replaces old player location with NULL
        findPlayerLocation(rooms, rows, columns);
    }
    ...
    if (choice == 2)
    {
        if (rooms[currentRowOfPlayer+1][currentColumnOfPlayer] != NULL)
        {
			//player moves to the right, but there is a wall
            if (rooms[currentRowOfPlayer+1][currentColumnOfPlayer]->getTileType() == '|' || rooms[currentRowOfPlayer+1][currentColumnOfPlayer]->getTileType() == 'o')
            {
                cout << "the way is blocked" << endl;
                return;
			...
			//player moves to the right and there is door, does nothing - its unlocked
            if (rooms[currentRowOfPlayer+1][currentColumnOfPlayer]->getTileType() == 'D')
            {
                cout << "";
            }
...
/*************************************
Description: Searches the array for @, and sets the current row and column to the player's location.
*************************************/
void Stairway::findPlayerLocation(Tiles ***inputRoom, int rows, int columns)
{
    for (int count = 0; count < rows; count++)
    {
        for (int count2 = 0; count2 < columns; count2++)
...
/*************************************
Description: Checks if the user casted the unlock spell in the room.
*************************************/
int Stairway::spellCheck()
{
	return spell;
}
